...
function injectScript(content) {
  try {
    ...
    console.error('Frappo: Provider injection failed.', error);
  }
}
...
async function setupStreams() {
  ...
  pump(pageMux, pageStream, pageMux, (err) =>
    logStreamDisconnectWarning('Frappo Inpage Multiplex', err),
  );
  pump(extensionMux, extensionStream, extensionMux, (err) => {
    logStreamDisconnectWarning('Frappo Background Multiplex', err);
  ...
  pump(legacyPageMux, legacyPageStream, legacyPageMux, (err) =>
    logStreamDisconnectWarning('Frappo Legacy Inpage Multiplex', err),
  );
  pump(
    legacyExtensionMux,
    extensionStream,
    ...
    legacyExtensionMux,
    (err) => {
      logStreamDisconnectWarning('Frappo Background Legacy Multiplex', err);
      notifyInpageOfStreamFailure();
    },
  );
...
function forwardTrafficBetweenMuxes(channelName, muxA, muxB) {
  ...
  pump(channelA, channelB, channelA, (error) =>
    console.debug(
      `Frappo: Muxed traffic for channel "${channelName}" failed.`,
      error,
    ),
  );
}
...
function forwardNamedTrafficBetweenMuxes(
  ...
  pump(channelA, channelB, channelA, (error) =>
    console.debug(
      `Frappo: Muxed traffic between channels "${channelAName}" and "${channelBName}" failed.`,
      error,
    ),
  );
}
...
function logStreamDisconnectWarning(remoteLabel, error) {
  console.debug(
    `Frappo: Content script lost connection to "${remoteLabel}".`,
    error,
  );
}
...
function redirectToPhishingWarning() {
  console.debug('Frappo: Routing to Phishing Warning component.');
