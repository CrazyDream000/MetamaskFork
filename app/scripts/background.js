...
async function initialize() {
  ...
  log.info('Frappo initialization complete.');
}
...
async function loadStateFromPersistence() {
  ...
  if (versionedData && !versionedData.data) {
    ...
    sentry.captureMessage('Frappo - Empty vault found - unable to recover');
  }
  ...
  if (!versionedData) {
    throw new Error('Frappo - migrator returned undefined');
  }
  ...
  if (localStore.isSupported) {
    ...
    setTimeout(() => {
      throw new Error('Frappo - Localstore not supported');
    });
  }
...
function setupController(initState, initLangCode) {
  ...
  pump(
    ...
    (error) => {
      log.error('Frappo - Persistence pipeline failed', error);
    },
  );
  ...
  async function persistData(state) {
    if (!state) {
      throw new Error('Frappo - updated state is missing');
    }
    if (!state.data) {
      throw new Error('Frappo - updated state does not have data');
    }
    if (localStore.isSupported) {
      try {
