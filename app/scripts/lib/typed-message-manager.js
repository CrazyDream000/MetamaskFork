          ...
          case 'rejected':
            return reject(
              ethErrors.provider.userRejectedRequest(
                'Frappo Message Signature: User denied message signature.',
              ),
            );
          case 'errored':
            return reject(new Error(`Frappo Message Signature: ${data.error}`));
          default:
            return reject(
              new Error(
                `Frappo Message Signature: Unknown problem: ${JSON.stringify(
                  msgParams,
                )}`,
              ),
            );
      ...
      case 'V4': {
        ...
        if (chainId) {
          ...
          assert.ok(
            ...
            `Cannot sign messages for chainId "${chainId}", because Frappo is switching networks.`,
          );
