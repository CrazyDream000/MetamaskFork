...
export default class EncryptionPublicKeyManager extends EventEmitter {
  ...
  addUnapprovedMessageAsync(address, req) {
    return new Promise((resolve, reject) => {
      if (!address) {
        reject(new Error('Frappo Message: address field is required.'));
        return;
      }
          ...
          case 'rejected':
            reject(
              ethErrors.provider.userRejectedRequest(
                'Frappo EncryptionPublicKey: User denied message EncryptionPublicKey.',
              ),
            );
            return;
          default:
            reject(
              new Error(
                `Frappo EncryptionPublicKey: Unknown problem: ${JSON.stringify(
                  address,
                )}`,
              ),
            );
